...
| **2.2** | **SVD is all `irlba`.** | `R/conos.R:185,288,344`; `R/integrations.R:313` | ~~Swap to `RSpectra::svds`~~ **DESCOPED (profiled non-win).** On a realistic per-pair PCA (3000×2000, 20 comps, single core = one fork) `irlba` 5.27s vs `RSpectra::svds` 5.11s (**1.03×, no speed win** — the per-pair fork is single-threaded so RSpectra's threading can't help), and the two give the **same subspace** (min cos principal angle 1.0000). So the only benefit is library consistency + dropping the `irlba` dep — not worth porting/validating ~4 SVD call sites with subtly different signatures (`center`/`right_only`/`reorth`) under regression risk this wave. Keep `irlba`; revisit as pure cleanup later. | — |
