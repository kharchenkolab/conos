...
## Plot all with depth with common color scale
...
plotAllWithGenesIntoFiles <- function (apps, genes, fileprefix, filesuffix, ...) {
    lapply(genes, function(gn) {
        filename <- paste0(fileprefix, gn, filesuffix);
        plotAllWithGene2(apps, gn, filename=filename, ...);
    })
}


## Compare a group of cells accross all apps
t.two.exp.comp <- function(rl1,rl2,cells,n1='test',n2='control',min.cells=30) {
    require(DESeq2)
    ...
    cts <- do.call(cbind,lapply(c(rl1,rl2),colSums))
    ...
    fun <- function(df, object, ...){
        ...
        df$ENSEMBL <- hwrite(as.character(df$ENSEMBL),link = paste0("http://www.genecards.org/cgi-bin/carddisp.pl?gene=GZMA", as.character(df$ENSEMBL)), table = FALSE)
        df
    }
    require(org.Hs.eg.db)
    des2Report <- HTMLReport(shortName = 'whole.vs.inv_T.cell.cyto', title = 'Cytotoxic T cells whole vs. involved', reportDirectory = "./reports")
    publish(dds,des2Report, pvalueCutoff=0.1, annotation.db=NULL, factor = colData(dds)$type, reportDir="./reports",modifyDF =list(modifyReportDF, fun))
    finish(des2Report)
    return(dds)
}
...
## Let's look at these subset specific markes on pavan's sample
