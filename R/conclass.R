...
Conos <- R6::R6Class("Conos", lock_objects=FALSE,
  public = list(
    ...
    #'
    #' @param k (default=15)
    #' @param k k.self (default=10)
    #' @param k k.self.weight (default=0.1)
    #' @param k alignment.strength (default=NULL)
    #' @param k space (default='PCA')
    #' @param k matching.method (default=='mNN')
    #' @param k metric (default='angular')
    #' @param k k1 (default=k)
    #' @param k data.type (default='counts')
    #' @param k l2.sigma (default=1e5)
    #' @param k var.scale (default=TRUE)
    #' @param k ncomps (default=40)
    #' @param k n.odgenes (default=2000)
    #' @param k neighborhood.average (default=FALSE)
    #' @param k neighborhood.average.k (default=10)
    #' @param k matching.mask (default=NULL)
    #' @param k exclude.samples (default=NULL)
    #' @param k common.centering (default=TRUE)
    #' @param k base.groups (default=NULL)
    #' @param k append.global.axes (default=TRUE)
    #' @param k append.decoys (default=TRUE)
    #' @param k decoy.threshold (default=1)
    #' @param k n.decoys (default=k*2)
    #' @param k score.component.variance (default=FALSE)
    #' @param k balance.edge.weights (default=FALSE)
    #' @param k balancing.factor.per.cell (default=NULL)
    #' @param k same.factor.downweight (default=1.0) 
    #' @param k k.same.factor (default=k)
    #' @param k balancing.factor.per.sample (default=NULL)
    ...
    getDifferentialGenes=function(clustering=NULL, groups=NULL, z.threshold=3.0, upregulated.only=FALSE, verbose=TRUE, plot=FALSE, n.genes.to.show=10, inner.clustering=FALSE,
      ...
      if (append.specificity.metrics) {
        ...
        de.genes %<>% names() %>% setNames(., .) %>%
          plapply(function(n) appendSpecificityMetricsToDE(de.genes[[n]], groups.clean, n, p2.counts=cm.merged, append.auc=append.auc), verbose=verbose, n.cores=n.cores)
      }
