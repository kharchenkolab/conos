...
#'
#' @param cms_list list of velocity files written out as cell.counts.matrices.rds files by running dropest with -V option
...
#' @export
#'
VelocityInfoConos <- function(cms_list, con, n.odgenes = 2e3, verbose = TRUE, min.max.cluster.average.emat=0.2, min.max.cluster.average.nmat=0.05, min.max.cluster.average.smat=0.01) {
  ...
  raw.count.matrix.merged <- lapply(con$samples, conos:::getRawCountMatrix) %>%
  ...
  cms_list <- lapply(cms_list, PrepareVelocity, rownames(raw.count.matrix.merged), colnames(raw.count.matrix.merged))
  # Keep only genes present in velocity files from all the samples
  common_genes <-  Reduce(intersect, lapply(cms_list, function(x) {rownames(x[[1]])}))
  cms_list = lapply(cms_list, function(x) {lapply(x, function(y) {y[row.names(y) %in% common_genes,]} )} )
  
  # Merge velocity files from different samples
  emat <- do.call(cbind, lapply(cms_list, function(x) {x[[1]]}))
  nmat <- do.call(cbind, lapply(cms_list, function(x) {x[[2]]}))
  smat <- do.call(cbind, lapply(cms_list, function(x) {x[[3]]}))
  ...
  # Get a pca results for all the samples from the conos object
  ...
  # Calculate the cell distances as in the original function
...
PrepareVelocity <- function(cms_file, genes, cells) {
  ...
  exon_cells <- colnames(cms_file$exon)
  intron_cells <- colnames(cms_file$intron)
  spanning_cells <- colnames(cms_file$spanning)
  # Only common cells between the 3 files and conos object
  common_cells <- intersect(exon_cells, intron_cells) %>% intersect(spanning_cells) %>% intersect(cells)
  
  cms_file$exon <- cms_file$exon[common_genes,common_cells]
  cms_file$intron <- cms_file$intron[common_genes,common_cells]
  cms_file$spanning <- cms_file$spanning[common_genes,common_cells]
  return(cms_file)
}

# Get a pca results for all the samples from the conos object
# This is a modification of the quickPlainPCA function
PCAforVelo <- function(r.n,data.type='counts',k=30,ncomps=100,n.odgenes=NULL,verbose=TRUE) {
  od.genes <- conos:::commonOverdispersedGenes(r.n, n.odgenes, verbose=verbose)
  if(length(od.genes)<5) return(NULL);
  
  if(verbose) cat('Calculating PCs for',length(r.n),' datasets ...')
  
  sm <- conos:::scaledMatrices(r.n, data.type=data.type, od.genes=od.genes, var.scale=TRUE, neighborhood.average=FALSE);
