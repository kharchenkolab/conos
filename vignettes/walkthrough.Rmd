...
A convenience function can be used to examine the compoistion of the clusters in terms of samples, sample entropy (middle), and cluster size (bottom):
...
Note 2: both functions `$plotGraph` and `$plotPanel` are based on the function `conos::embeddingPlot` and forward all visualization paramteres to this function. So, to get full list of the possible parameters see `?conos::embeddingPlot`.
...
This function returns probabilites of the cell belonging to each group. These probabilities can be used to estimate uncertainty of the labeling:
...
We now see that all our samples have been labelled automagically!
