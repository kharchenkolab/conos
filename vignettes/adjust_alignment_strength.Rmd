...
cellannot <- file.path(find.package('conos'),'extdata','cellannot.txt') %>%
  read.table(header=FALSE,sep='\t') %$% setNames(V2, V1)
```

Then we can preprocess the samples with [pagoda2](https://github.com/kharchenkolab/pagoda2), and perform alignment with `conos`:
...
In this dataset our samples are grouped by tissue, i.e. bone marrow (BM) vs.
cord blood (CB), so we can color by this factor:
