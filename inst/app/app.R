...
server <- function(input, output) { #plots under a slider 
  ...
  observeEvent(input$do, {
    memb <- dataInput()$memb
    gready.cut.groups <- paste("gready.cut.groups.",".clusters.rda",sep=as.character(input$N))
    names <- names(memb)
    memb <- as.character(memb)
    names(memb) <- names
    tempDir <- system.file("app", "temp", package = "conos")
    saveRDS(memb, paste(tempDir,gready.cut.groups,sep="/"))
  })
