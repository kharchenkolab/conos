...
**write** three samples to files (in practice they already exist). Here we use the anndata `.h5ad` and
lstar (zarr) formats, both written by pagoda2:
...
A Seurat `.h5seurat` (written by `SeuratDisk::SaveH5Seurat`) and a `.loom` file are read exactly the same
way — point the matching constructor at the file:
